  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Verify a claim against the SciFact verification model and dense evidence retrieval."
      />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionCard title="Claim">
          <Textarea
            ...
            placeholder="Enter a scientific claim or statement to verify..."
            aria-label="Claim"
            className="min-h-32 resize-none rounded-xl bg-background"
          />
          <p className="mt-3 text-xs text-muted-foreground">
            If evidence is omitted, the backend retrieves relevant SciFact evidence.
          </p>
        </SectionCard>
        ...
        <SectionCard title="Evidence (optional)">
          <Textarea
            ...
            placeholder="Paste evidence to verify, or leave blank for SciFact retrieval..."
            aria-label="Evidence"
            className="min-h-32 resize-none rounded-xl bg-background"
          />
          <p className="mt-3 text-xs text-muted-foreground">
            User-provided evidence is evaluated directly.
          </p>
        </SectionCard>
      </div>

      <Button
        className="mt-6 h-12 rounded-xl px-6 font-medium"
        disabled={loading}
        onClick={() => void verify()}
      >
        {loading ? (
          ...
          <SectionCard title="Prediction">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              {result.confidence_available
                ...
                : "Confidence unavailable because evidence was insufficient."}
            </p>
            ...
            {result.evidence.length ? (
              <div className="space-y-3">
                {result.evidence.map((item) => (
                  <div key={`${item.source}:${item.title}`} className="rounded-xl border border-border p-3">
                    <p className="text-sm font-semibold">{item.title}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{shortExcerpt(item.content)}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">No sufficiently relevant evidence was found.</p>
            )}
          </SectionCard>

          <SectionCard
            title={`Knowledge Graph (${result.knowledge_graph.nodes.length} nodes, ${result.knowledge_graph.edges.length} relationships)`}
            action={
          ...
          >
            {result.knowledge_graph.nodes.length ? (
              <InteractiveKnowledgeGraph
                data={result.knowledge_graph}
                height="560px"
              />
            ) : (
              <p className="text-sm text-muted-foreground">No evidence-derived relationships were found.</p>
            )}
          </SectionCard>
        </div>
      )}
